...
public interface AllocationRepository extends JpaRepository<Allocation, Long> {
	...
	@Query(value = "select  employee_id, employee_name, employee_email, project_name,\r\n"
			...
			+ "			join project p on a.project_id = p.project_id) as ranks where temp=1 ; "
			, nativeQuery = true)
...
//	List<Map<String, Object>> getAllocationHistoryByEmpEmail(@Param("email") String email);
//	
//	@Modifying(clearAutomatically=true)
//	@Transactional
//	@Query(value = "select e.employee_id, e.employee_name, e.employee_email, p.project_name, "
...
//			+ "join client c on a.client_id = c.client_id "
//			+ "join project p on a.project_id = p.project_id ",nativeQuery=true)
//	List<Map<String, Object>> getAllocationHistoryByEmpName(@Param("name") String name);

	@Modifying(clearAutomatically=true)
	@Transactional
	@Query(value = "select e.employee_id, e.employee_name, e.employee_email, p.project_name, "
			+ "p.project_department, p.project_location, a.is_active, c.client_name,a.start_date,a.end_date "
